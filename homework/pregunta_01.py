            ...
            with zipfile.ZipFile(os.path.join(input_path, file), "r") as z:
                for csv_file in z.namelist():
                    ...
                    with z.open(csv_file) as f:
                        ...
                        client_df["job"] = (
                            client_df["job"].str.replace(".", "").str.replace("-", "_")
                        )
                        client_df["education"] = (
                            client_df["education"]
                            .str.replace(".", "_")
                        ...
                        client_df["credit_default"] = (
                            client_df["credit_default"].map({"yes": 1}).fillna(0)
                        )
                        client_df["mortgage"] = (
                            client_df["mortgage"].map({"yes": 1}).fillna(0)
                        )
                        client_df.to_csv(
                        ...
                        campaign_df["campaign_outcome"] = (
                            campaign_df["campaign_outcome"].map({"yes": 1}).fillna(0)
                        )
